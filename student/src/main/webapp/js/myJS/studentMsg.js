...
$(function(){
			...
			if(rows.length>0){
				$.messager.confirm('警告','确认删除？',function(flag){
					if(flag){
						...
						$.post("studentMsgGet2.do",{delItems:JSON.stringify(delItem)},function(data,status){});
					}
				});
			}else{
	...
	$('#dg').datagrid({
		...
		onAfterEdit:function(rowIndex,rowData,changes){
			...
			$('#redo,#save').hide();
			$.post("studentMsgGet2.do",{itemInfo:JSON.stringify(rowData)},function(data,status){
			});
			obj.search();
		}
	});
